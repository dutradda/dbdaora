...
@pytest.fixture
def fake_hash_repository_cls(fake_entity_cls):
...
async def test_should_get_many(
    ...
    entities = [
        ...
        async for e in fake_service.get_many(
            ('other_fake', 'fake'), ('other_fake', 'fake2')
        )
    ]
...
async def test_should_get_many_from_cache(
    fake_service, serialized_fake_entity, fake_entity, fake_entity2, mocker
):
    fake_service.repository.memory_data_source.hgetall = (
        asynctest.CoroutineMock()
    )
    fake_service.cache["('other_fake', 'fake')"] = fake_entity
    fake_service.cache["('other_fake', 'fake2')"] = fake_entity2
    entities = [
        e
        async for e in fake_service.get_many(
            ('other_fake', 'fake'), ('other_fake', 'fake2')
...
async def test_should_get_many_from_fallback_after_open_circuit_breaker_with_fields(
    fake_service, fake_entity, fake_entity2
):
